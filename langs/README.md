...
2. **制約付き認識**（`python -m baronh recognize`）  
   音響モデルは持たない。仮名読み・IPA・正書法を、語形索引（辞書×形態）の最長一致で表層に落とす。これは小型のレキシコン FST である。Whisper の書き起こしをこの層へ渡せば、架空言語の「音声認識」になる。仮名プロキシでは `l` と `r` が同じラ行になるなど衝突が残るので、同じ読みでは主格・対格・具格を向格より先に採る。IPA の方が区別できる。
