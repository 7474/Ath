...
見出しのラテン転写は Wikipedia「アーヴ語」およびアース字母（`é ï œ ü ÿ`、二重字 `ai au eu`）に揃えます。Latin-1 資料が œ の代わりに書く `oe` は取り込み時に `œ` へ畳みます。語末の `oe`（`boe` 思う、`ramgoe` さまよう）は o 語幹に不定詞 `-e` が付いた形なので畳みません。掻き集め辞書の `&#339;` は HTML 実体として œ になります。ASCII の `e'`（`spe'nec`）は `é` へ、アースに無い `k/j/v/w/q/x` は `c/gh/bh/u/c/cs` へ畳みます。
